...
/*    if (!nh.getParam("manual_drive", manual_drive_))
    ...
    if(!nh.getParam("angular_z_max", angular_z_max_))
    {
        angular_z_max_ = 0.5;
    }

    if(!nh.getParam("linear_x_max", linear_x_max_))
    {
        linear_x_max_ = 0.5;
    }
...
{
    std_msgs::String lift_cmd_msg;

    loop_cnt++;

    if(loop_cnt == 20)
    {
    	lift_cmd_msg.data = "lift_down_cmd";
    	lift_req_pub.publish(lift_cmd_msg);
    }

    if(loop_cnt == 40)
    {
	loop_cnt = 0;
	lift_cmd_msg.data = "lift_up_cmd";
        lift_req_pub.publish(lift_cmd_msg);
    }
...
{
    // Init
    ros::init(argc, argv,"agv_teleop_diff_lift_node");
